  ...
  return (
    <form action={formAction} className="liquid-card flex flex-col gap-5 border border-border bg-card p-5 md:p-7">
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <div className="flex flex-col gap-1.5 sm:col-span-2"><label htmlFor="rfq-product" className={labelClass}>{f.product}</label><select id="rfq-product" name="product" defaultValue="" className={inputClass}><option value="" disabled>{f.productPlaceholder}</option>{products.map((product) => <option key={product.id} value={product.code}>{dict.products.items[product.id].name}</option>)}<option value="other">{f.productOther}</option></select></div>
