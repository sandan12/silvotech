...
export function RfqForm({ dict }: { dict: Dictionary }) {
  ...
  if (state.status === 'success') {
    return (
      <div className="flex flex-col items-start gap-3 border border-accent/40 bg-card p-8">
  ...
  return (
    <form action={formAction} className="flex flex-col gap-5 border border-border bg-card p-6 md:p-8">
      <div className="grid gap-5 sm:grid-cols-2">
        <div className="flex flex-col gap-1.5">
          <label htmlFor="rfq-company" className={labelClass}>
            {f.company} *
          </label>
          <input id="rfq-company" name="company" required maxLength={160} className={inputClass} />
        </div>
        <div className="flex flex-col gap-1.5">
          <label htmlFor="rfq-name" className={labelClass}>
            {f.name} *
          </label>
          <input id="rfq-name" name="name" required maxLength={160} className={inputClass} />
        </div>
        <div className="flex flex-col gap-1.5">
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <input id="rfq-country" name="country" maxLength={80} className={inputClass} />
        </div>
        <div className="flex flex-col gap-1.5">
          ...
          <select id="rfq-product" name="product" defaultValue="" className={inputClass}>
            ...
            {products.map((product) => (
              <option key={product.id} value={product.code}>
                {dict.products.items[product.id].name} ({product.code})
              </option>
            ))}
        ...
        <div className="flex flex-col gap-1.5 sm:col-span-2">
          <label htmlFor="rfq-size" className={labelClass}>{f.size}</label>
          <select id="rfq-size" name="size" defaultValue="" className={inputClass}>
            <option value="" disabled>{f.sizePlaceholder}</option>
            {standardSizes.map((size) => <option key={size} value={`${size} mm`}>{size} mm</option>)}
      ...
      {state.status !== 'idle' && (
        <p
          role="alert"
          className="flex items-start gap-2.5 border border-destructive/30 bg-destructive/5 p-3.5 text-sm leading-relaxed text-destructive"
        >
          <AlertTriangle className="mt-0.5 size-4 shrink-0" aria-hidden="true" />
          {state.status === 'invalid' ? dict.rfq.required : dict.rfq.errorText}
        </p>
      )}
