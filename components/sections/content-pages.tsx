...
export function DocumentsPage({copy}:{copy:SiteCopy;lang:Locale}) {
  return <><PageHero label={copy.nav.documents} title={copy.docs.title} lead={copy.docs.lead}/><section className="section"><div className="shell docs-grid">{copy.docs.items.map(x=><article key={x.title}><SoftGlyph kind="quality"/><h2>{x.title}</h2><p>{x.text}</p></article>)}</div></section></>;
}
