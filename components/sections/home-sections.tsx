...
const productPhotos: Array<string | null> = [
  '/media-new/hoses-and-tubing.webp',
  ...
  '/media-new/made-to-drawing.webp',
...
export default function HomeSections({ copy, lang }: { copy: SiteCopy; lang: Locale }) {
  return <>
    ...
    <section className="section"><div className="shell"><SectionHeading label={copy.benefits.label} title={copy.benefits.title}/><div className="benefits-grid">{copy.benefits.items.map((item,i)=>{const kinds:GlyphKind[]=['factory','custom','layers','quality','product','logistics'];return <article key={item.title}><SoftGlyph kind={kinds[i] ?? 'product'} size={46}/><h3>{item.title}</h3><p>{item.text}</p></article>})}</div></div></section>
...
function SectionHeading({ title, lead }: { label: string; title: string; lead?: string }) { return <div className="section-heading"><h2>{title}</h2>{lead&&<p>{lead}</p>}</div>; }
